...
(function($) {
    ...
    $.fn.flipLightBox = function(flb_options) {
        ...
        function flb_find() { if (flb_classnum == 1) { fgi_lb_ind_name = fgi_lb_name } else { fgi_lb_ind_name = fgi_lb_name;
                fgi_lb_ind_name += flb_classnum; } if ($('.' + fgi_lb_ind_name)[0]) { $('.' + fgi_lb_ind_name + ' > span').hide();
                flb_itemnum = 0;
                $('.' + fgi_lb_ind_name).each(function() { flb_itemnum = flb_itemnum + 1;
                    $(this).attr('class', fgi_lb_ind_name + '-' + flb_itemnum); if (flb_clickclass == null) { flb_clickclass = '.' + fgi_lb_ind_name + '-' + flb_itemnum; } else { flb_clickclass += ', .' + fgi_lb_ind_name + '-' + flb_itemnum; } });
                flb_classamount[fgi_lb_ind_name] = flb_itemnum;
                flb_classnum++;
                flb_find(); } }
        flb_find();
        if (flb_settings.lightbox_background_color == null) { flb_settings.lightbox_background_color = '#000000'; }
        ...
        function flb_get_image_width(src) { var flb_img = new Image();
            flb_img.src = src; return flb_img.width; }

        function flb_get_image_height(src) { var flb_img = new Image();
            flb_img.src = src; return flb_img.height; }

        function flb_iphone_check() { return ((navigator.platform.indexOf("iPhone") != -1) || (navigator.platform.indexOf("iPod") != -1)); }

        function flb_load_animation() { $("#flb-load").html('<div id="flb-loading-img"></div>').show();
            setTimeout(function() { $("#flb-loading-img").html('<br />Loading Image'); }, 500);
        ...
        $(flb_clickclass).click(function(e) { e.preventDefault();
            ...
            $("#flb-lightbox-text").html(''); if (flb_textcontent != null && flb_textcontent != '' && flb_settings.lightbox_text_status == 1) { if (flb_settings.lightbox_navigation_status == 1 && flb_total_images > 1) { $("#flb-lightbox-text").append('<br /><br />'); }
            ...
            $("<img />").attr('src', flb_image_name).on("load", function() { flb_in_img.onload = function() { $("#flb-load").html('');
                    ...
                    setTimeout(function() { $(window).resize();
                        ...
                        setTimeout(function() { if ($("#flb-lightbox-text").is(':hidden')) { if (navigator.userAgent.match(/iPad/i) || flb_iphone_check()) { $("#flb-lightbox-text").show(); } else { $("#flb-lightbox-text").slideToggle(); } } }, flb_settings.lightbox_flip_speed);
                        window.setTimeout(function() { $("#flb-lightbox-content").show();
                            $("#flb-lightbox-image").attr('src', flb_image_name).animate({ width: flb_new_width, height: flb_new_height, opacity: 1 }, { duration: flb_settings.lightbox_flip_speed / 2 }); if (flb_settings.lightbox_image_drag != 1) { $("#flb-lightbox-image").css({ 'user-drag': 'none', 'user-select': 'none', '-moz-user-select': 'none', '-webkit-user-drag': 'none', '-webkit-user-select': 'none', '-ms-user-select': 'none' }); } }, flb_settings.lightbox_flip_speed / 2);
                        ...
                        $("#flb-close").click(function() { $(document).unbind("keyup", flb_escape);
                            ...
                            setTimeout(function() { $("#flb-lightbox").fadeOut(500); if ($("#flb-lightbox-text").is(':visible')) { if (navigator.userAgent.match(/iPad/i) || flb_iphone_check()) { $("#flb-lightbox-text").fadeOut(); } else { $("#flb-lightbox-text").slideToggle(); } } }, flb_settings.lightbox_flip_speed / 2); });
                        window.setTimeout(function() { $("#flb-close").attr('src', flb_close_image);
                            clearTimeout(flb_loading); }, flb_settings.lightbox_flip_speed); if (flb_settings.lightbox_navigation_status == 1 && flb_total_images > 1) { if (flb_lightbox_data[1] < flb_total_images) { $("#flb-lightbox-text").prepend('<a href="next" id="flb-next-pic" style="' + flb_settings.next_and_back_image_text_style + '' + flb_box_sizing + '">' + flb_settings.next_image_text + '</a>');
                                $("#flb-next-pic").click(function(e) { $(document).unbind("keyup", flb_escape);
                                    $("#flb-close, #flb-next-pic, #flb-back-pic").unbind("click"); if ($("#flb-lightbox-text").is(':visible')) { if (navigator.userAgent.match(/iPad/i) || flb_iphone_check()) { $("#flb-lightbox-text").fadeOut(); } else { $("#flb-lightbox-text").slideToggle(); } }
                                    e.preventDefault();
                ...
                flb_in_img.src = flb_image_name; }).error(function() { $("#flb-load").html('');
                clearTimeout(flb_loading);
                setTimeout(function() { clearTimeout(flb_loading); }, 3000);
                $("#flb-lightbox").fadeOut(500);
                $("#flb-lightbox-content, #flb-lightbox-text").html('');
                $("#flb-lightbox-text").hide(); }); });
        $(window).resize(function() { flb_i = 1; if ($(window).height() < (flb_image_height + $('#flb-lightbox-text').height() + flb_h) + (flb_settings.lightbox_border_width * 2)) { flb_new_height = $(window).height() - (flb_settings.lightbox_border_width * 2) - $('#flb-lightbox-text').height() - flb_h;
                flb_new_width = flb_image_width / flb_image_height * flb_new_height;
                flb_i = 0 } if ($(window).width() < (flb_image_width + 60) + (flb_settings.lightbox_border_width * 2) && ($(window).width() - flb_image_width) < ($(window).height() - flb_image_height - ($('#flb-lightbox-text').height() + flb_h))) { flb_new_width = $(window).width() - (flb_settings.lightbox_border_width * 2) - 60;
                flb_new_height = flb_image_height / flb_image_width * flb_new_width;
                flb_i = 0 } if (flb_i == 1) { flb_new_width = flb_image_width;
                flb_new_height = flb_image_height; } if (flb_settings.lightbox_text_area_position == 'top' && flb_h == 70) { flb_placement = (($('#flb-lightbox-text').height() / 2) + 15) * 2 } else { flb_placement = 0 }
            ...
            $("#flb-close").attr('style', 'margin-top: ' + flb_distance_from_top + 'px; margin-left: ' + flb_total_lightbox_width + 'px; ' + flb_box_sizing + ''); if (flb_settings.flip_mode == 0) { flb_image_innit_width = flb_new_width; } else { flb_image_innit_width = 0; }; });
    }
})(jQuery);